...
func (e *Engine) sendResponse(serverResponse *http.Response) error {
	...
	clientResponse := &http.Response{
		...
		ContentLength: serverResponse.ContentLength,
	}

	// Write should close the response body after it is sent, but does not
	// guarantee that it will be closed if an error occurs.
